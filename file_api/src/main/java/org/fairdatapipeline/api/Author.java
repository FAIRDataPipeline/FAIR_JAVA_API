...
public class Author {
    ...
    Author(RestClient restClient) {
        this.registryAuthor = (RegistryAuthor) restClient.getFirst(RegistryAuthor.class, Collections.emptyMap());
    ...
    void SoniaAuthor(RestClient restClient) {
        // sonia's get_author gets the admin user, then find the author linked to this use by the user_author table.
        // not using this (yet) as my user_author table is empty.
        RegistryUsers u = (RegistryUsers) restClient.getFirst(RegistryUsers.class, Collections.singletonMap("username", "admin"));
        RegistryUser_author ua = (RegistryUser_author) restClient.getFirst(RegistryUser_author.class, Collections.singletonMap("user", u.getUrl()));

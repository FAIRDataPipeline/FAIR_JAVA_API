...
public class  FDP_ObjectListReader implements MessageBodyReader<FDP_ObjectList> {
    ...
    @Override
    public FDP_ObjectList<?> readFrom(Class<FDP_ObjectList> type,
                                      Type genericType,
                                      Annotation[] annotations, MediaType mediaType,
        ...
        try {
            ...
            JavaType ptype = om.getTypeFactory().constructParametricType(FDP_ObjectList.class, (Class) MyTypeParameter);
            return (FDP_ObjectList) om.readValue(entityStream, ptype);
        } catch (Exception e) {
            throw new ProcessingException("Error deserializing FDP_ObjectList",
                    e);
        }
    }
}

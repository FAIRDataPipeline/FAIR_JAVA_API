...
public class NumericalArrayImpl implements NumericalArray {
   ...
   * the added (top level) dimensions will have length 1. make sure that the dimensions that already existed
   * have the same length as in shape. (the first (top-level) lengths in shape are ignored)
   * ie. if current shape is {3, 4} and longershape is {5, 3, 4}, we will return a NumericalArray with shape {1, 3, 4}
   * (the array with shape {1, 3, 4} can we written to a variable with shape {5, 3, 4} with offsets {0, 0, 0} up to {4, 0, 0})
   *
  ...
  public NumericalArray matchShape(int[] longershape) {
    if(longershape.length <= this.shape.length) throw(new IllegalArgumentException("matchShape(): the asked for longershape must be longer than the current shape."));
    if(!Arrays.equals(this.shape, 0, this.shape.length, longershape, longershape.length-shape.length, longershape.length)) throw(new IllegalArgumentException("the non-padded/added right-hand-side dimensions in longershape must match current shape!"));
    Object r = nDArray;
  ...
  @Override
  public Object asObject() {return nDArray; }
